...
public class ThreeCubeRight extends SequentialCommandGroup {
  ...
  public ThreeCubeRight() {
    ...
    addCommands(
      ...
      RobotContainer.m_Drivetrain.resetEncoders().andThen(new ParallelCommandGroup(new DriveDist(150, 0, PidConstants.DRIVE_SPEED, PidConstants.kp_DRIVERIGHT, PidConstants.ki_DRIVERIGHT, PidConstants.kd_DRIVERIGHT), new flipArmParallel())),
      //Shoot cube 2
      RobotContainer.m_Arm.setPosition(1),
      //Drive cube 3
      RobotContainer.m_Drivetrain.resetEncoders().andThen(new ParallelCommandGroup(new DriveDist(100, 0, -PidConstants.DRIVE_SPEED, PidConstants.kp_DRIVERIGHT, PidConstants.ki_DRIVERIGHT, PidConstants.kd_DRIVERIGHT), new flipArmParallel())),
      //Turn to cube 3
      new TurnAngle(-45).withTimeout(1.5),
      //Drive past charge station 
      RobotContainer.m_Drivetrain.resetEncoders().andThen(new DriveDist(50, -120, -PidConstants.DRIVE_SPEED, PidConstants.kp_DRIVERIGHT, PidConstants.ki_DRIVERIGHT, PidConstants.kd_DRIVERIGHT)),
      RobotContainer.m_Drivetrain.resetEncoders().andThen(new DriveDist(50, -120, PidConstants.DRIVE_SPEED, PidConstants.kp_DRIVERIGHT, PidConstants.ki_DRIVERIGHT, PidConstants.kd_DRIVERIGHT)),
      //Turn to grid
      new TurnAngle(0).withTimeout(1.5),
      //Drive to grid
      RobotContainer.m_Drivetrain.resetEncoders().andThen(new ParallelCommandGroup(new DriveDist(100, 0, PidConstants.TURN_SPEEDRIGHT, PidConstants.kp_DRIVERIGHT, PidConstants.ki_TURNRIGHT, PidConstants.kd_DRIVERIGHT), new flipArmParallel())),
